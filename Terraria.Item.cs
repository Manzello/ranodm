...
public class Item : Entity
{
	...
	private bool IsAPrefixableAccessory()
	{
		if (accessory && !vanity)
		{
	...
	public void SetDefaults5(int type)
	{
		...
		case 3992:
			...
			handOnSlot = 20;
			handOffSlot = 12;
			SetShopValues(ItemRarityColor.Pink5, sellPrice(0, 15));
			break;
		case 3993:
		...
		case 4061:
			...
			SetWeaponValues(12, 6f);
			SetShopValues(ItemRarityColor.Blue1, sellPrice(0, 0, 30));
			break;
		case 4062:
			DefaultToStaff(731, 8f, 17, 5);
			SetWeaponValues(12, 2f);
			SetShopValues(ItemRarityColor.Blue1, sellPrice(0, 0, 30));
			break;
		case 4066:
			DefaultToMount(15);
		...
		case 4233:
			...
			break;
		case 4234:
			DefaultToPlacableWall(238);
			break;
		case 4235:
			DefaultToPlacableWall(239);
			break;
		case 4236:
			DefaultToPlacableWall(240);
			break;
		case 4237:
		...
		case 4347:
			...
			DefaultToMagicWeapon(876, 37, 15f);
			mana = 16;
			width = 40;
			height = 40;
			knockBack = 6f;
		...
		case 4502:
		case 4503:
			DefaultToPlacableWall((ushort)(246 + type - 4486));
			break;
		case 4504:
		case 4505:
			DefaultToPlacableWall((ushort)(264 + type - 4504));
			value = 250;
			break;
		case 4506:
		case 4507:
			DefaultToPlacableWall((ushort)(266 + type - 4506));
			break;
		case 4508:
			DefaultToPlacableWall((ushort)(268 + type - 4508));
			value = 250;
			break;
		case 4509:
		case 4510:
		case 4511:
			DefaultToPlacableWall((ushort)(269 + type - 4509));
			break;
		case 4512:
			DefaultToPlacableWall(274);
			break;
		case 4513:
		case 4514:
		case 4515:
		...
		case 4539:
		case 4540:
			DefaultToPlacableWall((ushort)(276 + type - 4513));
			break;
		case 4599:
		case 4600:
		case 4601:
		...
		case 4671:
			...
			break;
		case 4672:
			DefaultToWhip(841, 14, 0.5f, 4f);
			SetShopValues(ItemRarityColor.Blue1, buyPrice(0, 15));
			break;
		case 4673:
			autoReuse = true;
			useStyle = 1;
	...
	private void SetFoodDefaults(int type)
	{
		...
		case 2267:
			DefaultToFood(22, 22, 206, 28800);
			SetShopValues(ItemRarityColor.Blue1, buyPrice(0, 0, 30));
			break;
		case 4623:
	...
	public void UpdateItem(int i)
	{
		...
		if (!beingGrabbed)
		{
			...
			if (timeLeftInWhichTheItemCannotBeTakenByEnemies == 0 && Main.netMode != 2 && Main.expertMode && playerIndexTheItemIsReservedFor == Main.myPlayer && IsACoin)
			{
				GetPickedUpByMonsters(i);
			}
		...
		{
			int num3 = ItemID.Sets.ItemSpawnDecaySpeed[type];
		...
		if (type == 267)
		{
			...
			for (int j = 0; j < 200; j++)
			{
				if (Main.npc[j].active && Main.npc[j].type == 22)
				{
					if (Main.netMode == 2)
					{
						NetMessage.SendData(28, -1, -1, null, j, 9999f, 10f, -Main.npc[j].direction);
					}
					Main.npc[j].StrikeNPCNoInteraction(9999, 10f, -Main.npc[j].direction);
	...
	private void MoveInWorld(float gravity, float maxFallSpeed, ref Vector2 wetVelocity)
	{
		...
		bool num = Collision.WetCollision(position, width, height);
		if (Collision.honey)
		{
			honeyWet = true;
		}
	...
	private void GetPickedUpByMonsters(int i)
	{
		...
		for (int j = 0; j < 200; j++)
		{
			if (!Main.npc[j].active || Main.npc[j].lifeMax <= 5 || Main.npc[j].friendly || Main.npc[j].immortal || Main.npc[j].dontTakeDamage || NPCID.Sets.CantTakeLunchMoney[Main.npc[j].type])
			{
				continue;
			}
			float num = stack;
			...
			num *= num2;
			float num3 = Main.npc[j].extraValue;
			int num4 = Main.npc[j].realLife;
			...
			if (!(num3 < num) || !(num3 + num < 9.99E+08f))
			{
				continue;
			}
			...
			if (rectangle.Intersects(rectangle2))
			{
				...
				Main.npc[num8].extraValue += num7;
				if (Main.netMode == 0)
				{
					Main.npc[num8].moneyPing(position);
				}
				else
				{
					NetMessage.SendData(92, -1, -1, null, num8, num7, position.X, position.Y);
				}
				if (stack <= 0)
				{
					SetDefaults();
	...
	private void CombineWithNearbyItems(int myItemIndex)
	{
		...
		for (int i = myItemIndex + 1; i < 400; i++)
		{
			...
			if (!item.active || item.type != type || item.stack <= 0 || item.playerIndexTheItemIsReservedFor != playerIndexTheItemIsReservedFor)
			{
				continue;
			}
	...
	public bool CanCombineStackInWorld()
	{
		...
		if (createTile < 0 && createWall <= 0 && (ammo <= 0 || notAmmo) && !consumable && (type < 205 || type > 207) && type != 1128 && type != 530 && dye <= 0 && paint <= 0)
		{
			return material;
		}
		return true;
